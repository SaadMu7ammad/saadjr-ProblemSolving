...
//cerr << "Time taken : " << (float)clock() / CLOCKS_PER_SEC << " secs" << endl;
 ...
 ll mult(ll val,ll st=1) {
   
     if (val < 10&&val!=1) {//if num is <10 surely will not divisible by 
         return 0;
   }
     else if (st == val) {//stop
             return 1;
     }
     else if (st > val) {//without this if the number will keep increase till death Xd
         return 0;
     }
     else {
        
       return  mult(val, st*20) || mult(val, st * 10);//we have two pathes so will make or
                                                   // coz maybe the number must be mult by 20 first not 10 and so on
