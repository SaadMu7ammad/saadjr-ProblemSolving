...
int main()
{
    ...
    ll r, s; cin >> r >> s;
    if (2 * r > s&&sqrt(pow(s,2)+ pow( s,2))<=2*r) {//square inside
        cout << "Circle\n";
          /*
          draw a  circle of r=4 and a square of side=8 u will find that the circle inside the square
          then from fethagorth the diagonal of square will be the actuall indicator that inside or not
          diagonal side =sqrt( S^2 + S^2 )
          so if diagonal side less than or equal the (2*r)diameter that means the square inside the circle
          
          */
    }
    else if (2 * r <= s) {//circle inside
        cout << "Square\n";
