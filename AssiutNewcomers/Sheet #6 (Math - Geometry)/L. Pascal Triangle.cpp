...
for (int i = 1; i <= n; i++) {//for n lines
         int val = 1;
    for (int k = 1; k <= i; k++)//for i elements in each line
    {
        cout <<val<<" ";
        val = val * (i - k) / (k);//just a fixed formula ..actually u are not the one who will discover this 
                                    //juat try it on paper and enjoy the accepted msg
    }
          cout << "\n";
    }
}
