...
int main()
{
    ...
    ll a, b;
    cin >> a >> b;
    if (b < a) {//make A is a start always so it must be less than B
        swap(a, b);
    }
   // cout << a << b << "\n";
    ll n = b - a + 1;//no of numbers 
    cout << ((a + b)* (n))/2<<"\n";
    //to calc the even sequence
    ll start, end;
  ...
  //  cout << "st" << start << " " << end << "\n";
    //to cal the numbers between 2even's interval 
    //div'em directly
    //the above if is for preprocessing the condition
    cout << ((start +end)/2) *((end-start)/2+1) << "\n";

    cout << ((a + b) * (n)) / 2 - ((start + end) / 2) * ((end - start) / 2 + 1) << "\n";//tot-even=odd
