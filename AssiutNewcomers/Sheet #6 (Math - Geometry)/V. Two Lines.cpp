...
{//https://codeforces.com/group/MWSDmqGsZm/contest/223338/problem/V
    ...
    int x3, y3, x4, y4;
    cin >> x3 >> y3 >> x4 >> y4;
    // be away from double issues :)
    //so instead of this equation 
    // we multiply بسط الاول ف مقام الثاني و العكس
    //multiply numerator of slope 1 with denominator of second slope and vise versa
   /* int m1 = (y2 - y1) / (x2 - x1);
    int m2 = (y4 - y3) / (x4 - x3);*/
     int m1 =  (y4 - y3)* (x2 - x1);
    int m2 =   (y2 - y1)*(x4 - x3);
