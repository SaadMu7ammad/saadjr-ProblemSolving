...
int main()
{
    ...
    while (cin >> a) {
        cin >> b;
        if (gcd(a, b) == 1) {//using Gcd idea is be shown as trial of some other test cases that the number must dont has no factors between them
            cout << setw(10) << a << setw(10) << b <<"    " << "Good Choice\n\n";
        }
        else 
            cout << setw(10) << a << setw(10) << b << "    " << "Bad Choice\n\n";
   }
